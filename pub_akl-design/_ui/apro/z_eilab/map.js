...
$(document).ready(function() {
	...
	$('.ia').pageMaker({
		manageMode:{ recentDate : '20-12-01' }, //모든페이지완료시 recentDate : 'finishAll'
...
var iaLayout = function(){
	...
	var windowHeight = $(window).outerHeight(true),
		...
		remoteHeight = remote.outerHeight(true);
...
$.fn.pageMaker = function(options){
	...
	if( o.pageDefault ){
		...
		page.each(function(){
			var _this = $(this);
			_this.find('.row_modify').clone().insertAfter( _this.find('.info.date') );
			_this.children('.row').first().find('.row_modify').removeAttr('class').addClass('modify_wrap');
		});
	...
	if( o.manageMode ){
		...
		pageDate.each(function(){
			var _this = $(this),
				releaseDate = Number( _this.text().replace(/-/g,''));

			recentDate = Number( recentDate );
			...
			if( recentDate == 'finishAll' ){ // 모두완료
				...
				if( recentDate < releaseDate || isNaN( releaseDate ) ) _this.addClass('recent'); // 릴리즈예정
