﻿$(function(){
	...
	$('.js-gallery-link').on('click', function(e) {
		...
		if ( !(_this.attr('href')) ) { return true }
...
function suggestRolling(ta){
	...
	setInterval(function(){
		...
		ta.find('li').eq(_prv).animate({top:-_hh},300,function(){
			$(this).css({top:_hh});
		}).dequeue();
		ta.find('li').eq(_now).animate({top:0},300).dequeue();
		_prv = _now;
	},5000);
};
//2019-08-28 메인 : 검색 닫기
...
function GNB_SubDepth_Control(){
	...
	if(oneDepthCon.length == 0){ return; }
	...
	$('.top-menu-links.desktop-structure-modified .nav-list-item').on('mouseenter', function() {
		...
		$(this).find('.overlay-menu-container').css({'height':'auto'}).addClass('in');
...
function layerPopOver( btnOpenLayer , targetLayer ){
	...
	/* 레이어 닫기 */
	var btn_close = 'a[class*=_close], [data-btn-close="closeLayer"], .cClose, .closeCbox, .modal-close-icon, .btnClose, .btn-modal_close'; //20220105 수정
...
$(function() {
	...
	function giftSlideTabSet(){
		if ($(window).width() < 768){
			$('.gift-home-slide').each(function(){
				...
				$('.slide-tab ul', this).css({width:gifttabsWidth});
			});
		}
	}
	...
	if ($('.gift-home-slide').length > 0) {
		...
		$slider.on("afterChange", function (slick, slide) {
			...
			if ($(window).width() < 768){
				if (current == length-1){
					if(length%2 != 0){paddingBuffer(270, 'easeOutBack');}
				}else if(current == length-2){
					if(length%2 == 0){paddingBuffer(128, 'easeOutBack');}
				}else if(current == 0){
...
var mobile_navigation = {
	obj:{
		...
		callback:function(that){return true}
	},
	...
	set_submenu:function(obj){
		...
		if (  obj.closest('dd').hasClass('business') ) {
			...
			if ( li_idx <= 4 ) {
				$(">ul", obj).css({ 'top': line1 + 'px' });	
			} else if( li_idx >= 5 && li_idx < 9 ){ 
				$(">ul", obj).css({ 'top': line1 + line2 + 'px' });	
			} else if( li_idx >= 9 ){
				$(">ul", obj).css({ 'top': line1 + line2 + line3 + 'px' });	
			}	
		}	else {
			$(">ul", obj).css({//하위메뉴 좌표체크 및 활성화
				'top':(obj.height()*Math.ceil(li_idx/4))+'px'
	...
	hide_Menu:function(obj){
		obj.each(function(){//초기화
			$(this).removeClass('on').css({'margin':0});
			$("li", $(this)).removeClass('on').css({'margin':0});
		});
	},
	exe_Menu:function(){
		var _this = this;
