...
function GNB_SubDepth_Control(){
	var oneDepthLink = $('.js-overlay-menu-toggle-desktop');
	var oneDepthCon = $('[id^=overlay-menu-wrapper]');
	var closeBtn = $('.search-results-close');
	if(oneDepthCon.length == 0){ return; }

	// 1depth click
	oneDepthLink.removeAttr('data-toggle').on('click', function() {
		if ( $(this).next('.overlay-menu-container').is(':visible')) {
			oneDepthCon.collapse('hide');
			$('.top-menu-links.desktop-structure-modified').removeClass('js-click-menu');
		} else {
			oneDepthCon.collapse('hide');		
			$(this).next('.overlay-menu-container').collapse('show');
			$(this).find('.tab-header-item').first().find('.tab-toggle').trigger('click');
			$('.top-menu-links.desktop-structure-modified').addClass('js-click-menu');
		}
	});

	// 1depth hover
	$('.top-menu-links.desktop-structure-modified .nav-list-item').on('mouseenter', function(e) {
		e.stopPropagation();
	...
	$('.top-menu-links.desktop-structure-modified .nav-list-item').on('mouseleave', function(e) {
		e.stopPropagation();
		oneDepthLink.addClass('collapsed');
		oneDepthCon.removeClass('in');
	});
