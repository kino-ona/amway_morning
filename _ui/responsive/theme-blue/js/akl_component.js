...
$.fn.uxToast = function (options) {
	...
	var offsetCalc = function () {
		if ($(window).width() > 768) {
			...
			var btn = $.extend($toastBtn.offset(), {
				w: $toastBtn.width(),
				h: $toastBtn.height()
			});
			...
			$(this).off("click").on("click", function (e) {
				...
				if ($dropdown.hasClass('list_minishop')) {
					...
					if (settings.swapTxtToTag === true) {
						...
						$btntxtfield.text(txt).addClass('uxe-selected');
					}
				};
...
var AP = function () {
	...
	var searchHandler = function () {
		searchMenu.on('click', function () {
			...
			/* S : 검색 레이어 펼쳐진 상태로 카테고리 메뉴 선택하면 검색 레이어 닫히기 */
			$('.layer-area_container .layer-area_content').css('padding-top', '55px');

			if ($('.layer-area_container').hasClass('is-search-opened')) {
				$('.depth-menu-area a.swiper-slide, .prd_search_form_box .btn-link').on('click', function () {
					$('.layer-area_container').removeClass('is-search-opened');
					$('.list_view-option .btn_prd_search').css('display', 'inline-block');
					$('.layer-area_container .layer-area_content').css('padding-top', '0');
				});
			}
			/* E : 검색 레이어 펼쳐진 상태로 카테고리 메뉴 선택하면 검색 레이어 닫히기 */
		});
		// 연관검색어
...
var productStickyMenu = function () {
	...
	$(window).on('scroll load resize', function () {
		if (productStickyMenu.onlyClick === true) {
			return
		}
		var winScrollTop = $(this).scrollTop(),
			headerHeight = $('.main-header').height();
...
$(function () {
	...
	$(document).on("click", ".btn_floating", function (e) {
		...
		if (!$(this).hasClass('active')) {
			$('.akl .renewal_floating-type ~ #footer .scrollTopWrapper').css('bottom', '140px'); // 20220712 챗봇 위치 변경
			$('.akl .renewal_floating-type ~ .box-chatbot').css('bottom', '55px'); // 20220712 챗봇 위치 변경
		} else {
			$('.akl .renewal_floating-type ~ #footer .scrollTopWrapper').css('bottom', '185px'); // 20220712 챗봇 위치 변경
			$('.akl .renewal_floating-type ~ .box-chatbot').css('bottom', '100px'); // 20220712 챗봇 위치 변경
		}
	});

	// 20220714 챗봇 그만보기 버튼 동작
	$('.btn-chatbot-hide').on('click', function () {
		$(this).closest('.box-chatbot').css('display', 'none')
	});
	...
	$btnAco.on('click', function () {
		if (!$(this).parent().parent().hasClass('active')) {
			...
			$('html, body').animate({
				scrollTop: offset.top - headerHeight - tabHeight
			}, 0);
		}
	...
	$btnCart.on('click', function () {
		if ($('#layer-addProduct').length >= 1) {
			return
		} // 2020-11-03 제품추가팝업 호출시 
		var _self = $(this);
		if (!_self.is('.active')) {
			_self.addClass('active');
			$layerBottom.addClass('is-opened');
