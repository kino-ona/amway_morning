﻿$(function () {
	...
	//푸터 > 위로이동
	onScroll_TOP(); // 20221017 위치 수정
	/* S : 20221017 추가 */
    setTimeout(function () {
		if($(document).scrollTop() != 0){
			//$('.scrollTopWrapper .toTop').addClass('totop_fade');
			//$('.scrollTopWrapper .toTop').css('opacity','1'); // 20221025 수정
			$('.scrollTopWrapper').css('display','block'); // 20221109 수정
		}
	}, 600); 
    /* E : 20221017 추가 */
	...
	//2019-09-23 메인개편 : ios, android 체크
	var chkAgentPhone = navigator.userAgent.toLowerCase();
	if (chkAgentPhone.indexOf("iphone") > -1 || chkAgentPhone.indexOf("ipad") > -1 || chkAgentPhone.indexOf("ipod") > -1) {
		$('html').addClass('ua-ios');
	} else if (chkAgentPhone.match('android') != null) {
	...
	$('.js-gallery-link').on('click', function (e) {
		...
		var _this = $(this);
		var videoLink = _this.attr('href');
		var videoTitle = _this.attr('title') || '암웨이 비디오';
		var videoText = _this.data('text') || '';

		if (!(_this.attr('href'))) {
			return true
		}
		if ($('.layerWrapper.js-gallery-layer').length > 0) {
			var _videoWrapper = $('.layerWrapper.js-gallery-layer');
			_videoWrapper.find('.headline-text').html(videoTitle);
			_videoWrapper.find('.video-wrapper').next().html(videoText);
			_videoWrapper.find('iframe').attr('title', videoTitle);
			_videoWrapper.find('iframe').attr('src', videoLink);
			setTimeout(function () {
				layerPopOver('', '.js-gallery-layer');
			}, 300);
		} else {
			var videoLayerHTML = '<div class="layerWrapper js-gallery-layer"><div class="videobox lynx-modal-window--opened directbox typeVideoView"><div class="video-pop"><div class="cboxTitle"><div class="headline"><span class="headline-text">' + videoTitle + '</span></div></div><div class="bonusPaymentDirectDeposit"><div class="bonus-payment-method__wrapper"><div class="popup-video-preview__content"><div class="video-cont"><div class="video-wrapper"><iframe width="100%" height="100%" title="' + videoTitle + '" src="' + videoLink + '" frameborder="0"></iframe></div><p class="mgt10">' + videoText + '</p></div></div></div></div><a href="#" class="cClose cboxClose js-gallery-close" title="닫기"><span class="modal-close-icon"></span></a></div></div></div>';
			$('body').append(videoLayerHTML);
			layerPopOver('', '.js-gallery-layer');
		}
	});
	...
	$('.js-gallery-close').on('click', function (e) {
		var _videoWrapper = $('.layerWrapper.js-gallery-layer');
		_videoWrapper.find('.headline-text').html('');
		_videoWrapper.find('.video-wrapper').next().html('');
		_videoWrapper.find('iframe').attr('title', '');
		_videoWrapper.find('iframe').attr('src', '');
	})

	/*  2019-11-07 NQ검사 추가항문 (기타) */
	$('.etc input[type="text"]').on('focus', function () {
		$(this).closest('label').click();
	});

	// 2019-11-18 : 선물몰 받으시는 분 체크
	var $rdo_list = $('.shipping-list-order');
	$('.selfInsertWrap').find('label').on('click', function () {
		if ($('input[id="selfInsertAddress"]').is(":checked") != true) {
			$rdo_list.addClass('addDisabled');
		} else {
			$rdo_list.removeClass('addDisabled');
		}
	});

	/* 2019-11-25 아이디어톡톡 정렬조건 on&off 효과 */
	$('.list_sort_type li').on('click', function () {
		$(this).siblings().removeClass('on');
		$(this).addClass('on');
	});

	/* 2019-12-16 locationBox 10자이상 말줄임 / 수정  */
	// textEllipsis('.locationBox li a', 9, true);

	/*툴팁 클릭 후 다른 툴팁 클릭 시 다른 툴팁 닫힘*/
	$('.toolTip-wrapper.tooltipNew .btn-tooltip').click(function (e) {
		var siblingTooltip = $(this).closest('.form-section-group').siblings('.form-section-group').find('.tooltipNew');
		if (siblingTooltip.hasClass('open')) {
			$(siblingTooltip).removeClass('open').find('.tooltip-content').hide();
		}
	});
...
/* 2019-12-16 locationBox 10자이상 말줄임 / 수정 */
function textEllipsis(target, length, mobile) {
	var exe_Ellipsis = function (target) {
...
/* S : 20230308 미니대시보드 이름 영역 수정 (Sprint3, 상시모니터링 No25) */
textEllipsis('.mini-dashboard a.myAccount span.user-name', 9, false);		
var hd_minidb_name = $('.mini-dashboard a.myAccount span.user-name');
hd_minidb_name.css({'display':'inline-block', 'width':hd_minidb_name.width() + 3});

/*setTimeout(function () {
	textEllipsis('.mini-dashboard a.myAccount span.user-name', 9, false);

	var hd_minidb_name = $('.mini-dashboard a.myAccount span.user-name');
	hd_minidb_name.css({'display':'inline-block', 'width':hd_minidb_name.width()});		
}, 300);*/
/* E : 20230308 미니대시보드 이름 영역 수정 (Sprint3, 상시모니터링 No25) */

/** ------------------------------------
 *  공통
 *  ------------------------------------
 */
//2019-08-19 메인 : 검색버튼 스크롤
function moSearchCtrl() {
	var _prvTop = $(window).scrollTop(),
		_nowTop = $(window).scrollTop(),
		_search = $('.search-btn-container'),
		_content = $('main'),
	...
	function classSet() {
		_nowTop = $(window).scrollTop();
		_footerTop = $('.quick-links').offset().top;
		if (_nowTop < 130) {
			_search.removeClass('up');
			_content.removeClass('up');
		} else if (_nowTop > _footerTop - _winH) {
			_search.addClass('up');
			_content.addClass('up');
		} else {
			if (_nowTop > _prvTop && !_search.hasClass('up')) {
				_search.addClass('up');
				_content.addClass('up');
			} else if (_nowTop < _prvTop && _search.hasClass('up')) {
				_search.removeClass('up');
				_content.removeClass('up');
			}
		}
...
// 2019-08-19 메인 : 탑배너 닫기
function topBannerClose() {
	$('.akl-topbanner').hide();
	$('body').removeClass('topbanner');
};
// 2019-08-19 메인 : 추천검색어 롤링
function suggestRolling(ta) {
	var _now = 0,
		_prv = 0,
		_max = ta.find('li').length,
		_hh = 32;
...
//2019-09-30 선물홈 버튼피드백
function btnFeedbackEffect(ta) {
	ta.on('mousedown touchstart', function () {
		$(this).addClass('active');
...
function onScroll_TOP() {
	// S : 20221012 스크롤 방향감지 TOP버튼 수정
	var scrollDIV = '<div id="ScrollTopArea" class="scrollTopWrapper"><button type="button" class="toTop">TOP(위로이동)</button></div>'; 
	// E : 20221012 스크롤 방향감지 TOP버튼 수정
	...
	function pageScrollTOP() {
		...
		//var lastScrollTop = 0;
		//var delta = 5;
		var btnScrollTop = document.querySelector('.scrollTopWrapper'); // 20221109 추가
		var fixBox = document.querySelector('.scrollTopWrapper .toTop');
		var didScroll;
		...
		function hasScrolled() {
			...
			/* S : 20221025 수정 */
			if (st > lastScrollTop) {
				//fixBox.classList.remove('totop_fade');
				//fixBox.style.opacity = '0';
				//fixBox.style.display = 'none'; // 20221109 수정
				//btnScrollTop.style.display = 'none'; // 20221109 추가
				$('.scrollTopWrapper').css('display','none'); // 20221109 추가
			} else {
				if(st + $(window).height() < $(document).height()) {
					if(st < 10) {
						...
						//fixBox.classList.add('totop_fade');
						//fixBox.style.opacity = '1';
						//fixBox.style.display = 'block'; // 20221109 수정
						//btnScrollTop.style.display = 'block'; // 20221109 추가
						$('.scrollTopWrapper').css('display','block'); // 20221109 추가
					}
				}
			}
			/* E : 20221025 수정 */
...
function MAIN_newHotBrand() {
	...
	_subBannerList.each(function () {
		...
		//이미지 BG에 넣기
		$(this).css({
			'background-image': 'url(' + _itemsIMG[0] + ')'
		});
		$(this).owlCarousel({
				loop: false,
				nav: true,
			...
			.on('initialized.owl.carousel , changed.owl.carousel', function (e) {
				...
				$(this).css({
					'background-image': 'url(' + _itemsIMG[active_ITEM] + ')'
				});
			});
	});
}
...
function tabsTgg_Control() {
	...
	if (!_tabsToggles.length || _exceptObject.length) {
		return;
	}
	...
	_tabsToggles.each(function () {
		...
		function $tabWidthCheck() {
			...
			if (_winWidth < 769) {
				...
				if (_winWidth > children_sum) {
					_UL.removeClass('scroll-tab width-auto');
				}
				if (_winWidth < children_sum) {
					_UL.addClass('scroll-tab');
				}
			} else {
				_UL.removeClass('scroll-tab');
				if (child_each_sum < children_sum) {
					_UL.removeClass('width-auto');
				}
				if (_winWidth < children_sum || _winWidth < outerWrapperWD || children_sum > outerWrapperWD) {
					_UL.addClass('width-auto');
				}
				if (_winWidth > children_sum) {
					_UL.removeClass('width-auto');
				}
...
function HEADER_stopPropagation() {
	if (!$('.new-v2').length) {
		//event bubbling 막기
		var stopPropagation_target = '.js-my-account-menu, .js-mini-cart-link, .auto-suggestion-popover, .popoverword, #shoppingcar-drop-content, #login-drop-content'; // 190409_수정
		$(stopPropagation_target).on('click', function (e) {
			e.stopPropagation();
		});

		// 190409_추가
		$("[id^=overlay-menu-wrapper]").on('click', function (e) {
			e.stopPropagation();
		});
		//html 클릭시 열린 레이어 닫기
...
function HEADER_MenuShop() {
	...
	if (isIE() === 9 && isIE() !== 10) {
		var overlayMenu = $('[id^=overlay-menu-wrapper]'); // 190409_수정
	...
	//쇼핑메뉴 닫기버튼 190409_수정
	var _btnLayerClose = $('[id^=overlay-menu-wrapper]').find('.search-results-close');
	_btnLayerClose.on('click', function () {
		$('[id^=overlay-menu-wrapper]').removeClass("in");
...
function HEADER_miniDashboard() {
	...
	if (!$('.new-v2').length) $('.top-search').removeClass('search-open'); //검색어 입력영역 //2019-08-28 메인 : 메인 개편 시 삭제
	if ($('.new-v2').length) $('.new-search-cont').removeClass('open'); // 2019-08-28 메인 : 추가
}
...
function navToggling() {
	...
	if (window.innerWidth < 768) {
		...
		twoDepth.off().on('click', function (e) { // 2depth event
			if ($(this).not('.no-children').length > 0) { //2019-08-19 메인 : 검색버튼 스크롤
...
function HEADER_MOB_search() {
	$(".mobile-search-btn").click(function () {
		//검색어 입력영역
		if (!$('.new-v2').length) { //2019-08-28 메인 : 메인 개편 시 삭제
			$('.top-search').hasClass('search-open') ? $('.top-search').removeClass('search-open') : $('.top-search').addClass('search-open');
		}
		$('header').removeClass('userinfo-open mincart-open');
		$(".dropdown").removeClass("open");
		if ($('.new-v2').length) $('.new-search-cont').addClass('open'); //2019-08-28 메인 : 추가
		if (!$('.new-v2').length) $('.js-site-search-input').trigger('click').focus(); // 190327_추가 모바일 검색 선택시 바로 입력 가능하게 // 2019-08-28 메인 : 개편 시 삭제
		if ($('.new-v2').length) $('.js-site-search-input').focus(); //2019-08-28 메인 : 수정
	});
...
function cartSummaryFixed() {
	if (!$('.cart-content-wrapper .shopping-cart-total-wrapper').length) {
		return;
	}
...
function videoSize() {
	...
	// 암웨이 소개 썸네일 슬라이드 버그 수정(2020-07-23)
	if (!PDP_IMGs.find('.video-wrapper').length) return;
...
function videoWrapperSize() {
	if (!$('.layerWrapper #colorbox .pop-gallery').length) {
		return;
	}
	...
	function layerPDP_gallery() {
		...
		$('.pop-gallery').find('.video-wrapper').css({
			top: topSpace,
			width: videoboxWidth,
			margin: '0 auto'
		});
...
function SOP_PDP_viewBundle() {
	if ($('.product-list-item.js-shopping-cart-list-item').length) {
		return;
	} //장바구니에서 return;
...
function MyOrderLIST_row_bgcolor() {
	...
	myOrderList.each(function () {
		...
		/* S : 나의 주문내역 list 하위메뉴 배경색 관련 js 추가 */
		if ($('.content-list').children().first().hasClass('sub-list')) {
			if ($('.content-list').children('.sub-list:eq(0)').hasClass('row-bg')) {
				$('div.con-list-item:odd').addClass('con-list-item__gray');
				$('div.con-list-item:even').addClass('con-list-item__white');
			} else {
				$('div.con-list-item:odd').addClass('con-list-item__white');
				$('div.con-list-item:even').addClass('con-list-item__gray');
			}

			if (LIST_ROW.hasClass('con-list-item__gray')) {
				$('.con-list-item__gray').nextUntil('.con-list-item').addClass('row-bg');
			}
			if (LIST_ROW.hasClass('con-list-item__white')) {
				$('.con-list-item__white').nextUntil('.con-list-item').removeClass('row-bg');
			}
		}
		/* E : 나의 주문내역 list 하위메뉴 배경색 관련 js 추가 */
	});
}
...
function myPage_EqualHeight() {
	...
	function colHeightResizeReSize() {
		...
		_UL.each(function (i) {
			...
			if (panel.length) {
				panel.find('.mypage-block-list ul>li>div').css({
					height: hiddenHeight
				});
			}
		});
 ...
 * -------------------------------------------
 */
// 190207_데스크탑 GNB 구조 개편 190409_수정 메뉴 중복 버그 수정
function GNB_SubDepth_Control() {
	var oneDepthLink = $('.js-overlay-menu-toggle-desktop').removeAttr('data-toggle');
	var oneDepthCon = $('[id^=overlay-menu-wrapper]');
	var closeBtn = $('.search-results-close');
	if (oneDepthCon.length == 0) {
		return;
	}

	// 1depth click
	oneDepthLink.on('click', function () {
		if (oneDepthCon.is(':visible')) {
			oneDepthLink.addClass('collapsed');
			oneDepthCon.removeClass('in');
		} else {
			oneDepthLink.addClass('collapsed');
			oneDepthCon.removeClass('in');
			$(this).removeClass('collapsed');
			$(this).next().addClass('in');
			$(this).next().find('.tab-header-item').first().find('.tab-toggle').trigger('click');
		}
	});

	// 1depth hover
	$('.top-menu-links.desktop-structure-modified .nav-list-item').on('mouseenter', function () {
		oneDepthLink.addClass('collapsed');
		oneDepthCon.removeClass('in');
		$(this).find('.js-overlay-menu-toggle-desktop').removeClass('collapsed');
		$(this).find('.overlay-menu-container').css({
			'height': 'auto'
		}).addClass('in');
		$(this).find('.tab-header-item').first().find('.tab-toggle').trigger('click');
	});

	$('.top-menu-links.desktop-structure-modified .nav-list-item').on('mouseleave', function () {
		oneDepthLink.addClass('collapsed');
		oneDepthCon.removeClass('in');
	});

	var $SubDepthBox = $('[id^=gnbSubDepthBox]');
	...
	/* 2depth click hover */
	var $menuLeftTab = $('[id^=overlay-menu-wrapper] .tab-header-list').find('a.tab-toggle');
	$menuLeftTab.each(function () {
		var _MenuID = $(this).attr('href');
		if (_MenuID == '#' || _MenuID[0] != '#') {
			_MenuID = '';
		}
		$(this).addClass('no-children');

		if ($SubDepthBox.find(_MenuID).length > 0) {
			$(this).removeClass('no-children').on('click mouseenter', function (e) {
				e.preventDefault();
	...
	closeBtn.removeAttr('data-toggle').click(function () {
		oneDepthLink.addClass('collapsed');
		oneDepthCon.removeClass('in');
	})
...
function quickLinks() {
	...
	//#accordion 메뉴위치 정렬하기
	//2019-08-19 메인 : 푸터 메뉴 정렬 메인 개편 후 사용X
	if (!$('.new_v2').length) qkLinkAlign();
	$(window).resize(function () {
		if (!$('.new_v2').length) qkLinkAlign();
...
function loadingLayer() {
	var loadImg = $('<div class="loading box_renew-loading" id="loading"><div class="loading-wrap"><span class="box_loading-area"><img src="/_ui/responsive/theme-blue/images/akl_common/img_loading-renewal.gif" alt="로딩중"></span></div></div>'); /* 2020-03-25 로딩 리뉴얼 마크업 변경 */
	if (loadImg.length) {
		$('#loading').remove();
	}
	$('body').append(loadImg);
...
function layerPopOver(btnOpenLayer, targetLayer) {
	...
	var $layerOverSize = $(targetLayer).find('.typeOverSize'); //풀캘린더 : 2019-07-11 추가
	...
	function setPosition() {
		...
		function _layerTypeBasic() {
			...
			// 모바일 전체 크기 팝업: S
			if ($layerContentBox.is('.max-height') || $layerContentBox.is('.clearStyle')) {
				if (_winWidth < 769) {
					$layerContentBox.removeAttr('style')
				}
			}
			// 모바일 전체 크기 팝업: E
		}
		...
		/* TYPE 4 : 달력 : 2019-07-11 추가 */
		function _layerTypeOverSize() {
			//모바일 전체화면
			var targetWidth = $layerOverSize.width();
			var targetHeight = $layerOverSize.height();
		...
		_layerTypeOverSize(); //풀캘린더 2019-07-11 추가
	}
	...
	/* 레이어 닫기 */
	var btn_close = 'a[class*=_close], [data-btn-close="closeLayer"], .cClose, .closeCbox, .modal-close-icon, .btnClose'; //20220105 수정 //20240325 운영/UAT 현행화(클래스 .btn-modal_close 삭제)
	...
	if (elTagName === tagA) {
		if (event) { // 2020-09-14 : event is undefined 에러 대응      
			if (isIE() === 9 || isIE() === 10) {
...
function msgboxCommon(msgboxType, msgboxContent, functionName, initialHeightDate) {
	msgboxCounter++;
	$.msgbox({
		id: msgboxCounter,
		type: msgboxType,
		content: (msgboxContent + "").replace(/[\n|\r]/g, "<br/>"),
		initialHeight: initialHeightDate,
		onOpen: function () {
...
(function ($, undefined) {
	...
	MSGBOX.prototype = {
		...
		_assemble: function () {
			...
			this.$icons.prev = createElement('a', this.options.prefix + '-prev', iconCss, {
				href: 'javascript:;',
				title: _('Prev', this.options.lang)
			});
			this.$icons.next = createElement('a', this.options.prefix + '-next', iconCss, {
				href: 'javascript:;',
				title: _('Next', this.options.lang)
			});
			this.$icons.play = createElement('a', this.options.prefix + '-play', iconCss, {
				href: 'javascript:;',
				title: _('Play/Pause', this.options.lang)
			});
			this.$icons.min = createElement('a', this.options.prefix + '-min', iconCss, {
				href: 'javascript:;',
				title: _('Minimize', this.options.lang)
			});
			this.$icons.max = createElement('a', this.options.prefix + '-max', iconCss, {
				href: 'javascript:;',
				title: _('Maximize', this.options.lang)
			});
			this.$icons.close = createElement('a', this.options.prefix + '-close', iconCss, {
				href: 'javascript:;',
				title: _('Close', this.options.lang)
			});
		...
		_loadImg: function ($handler, callback) {
			...
			var imgload = function () {
				...
				if (that.options.photoScaled) { // scaled to height and width
					...
					var realWidth = imgWidth * ratio,
						realHeight = imgHeight * ratio;
					that.$img.css({
						position: 'absolute',
		...
		_restoreFromMin: function (callback) {
			...
			this.animate(this.minimized, function () {
				...
				that.$icons.min
					.unbind('click.min.' + that.options.prefix)
					.one('click.min.' + that.options.prefix, function () {
						that.min();
					});
				that.enableDrag();
				that.enableResize();
		...
		_min: function (where, callback) {
			...
			this.animate(where, function () {
				...
				that.$icons.min
					...
					.one('click.restore.' + that.options.prefix, function () {
						that.restore();
					});
			...
			this.$resize.show().bind('mousedown.resize.' + this.options.prefix, function (e) {
				...
				$(document).bind('mouseup.resize.' + that.options.prefix, function (e) {
					...
					that.animate({
						width: width,
						height: height,
						top: that.resizeData.top,
						left: that.resizeData.left
					}, undefined, 0);
		...
		open: function (callback) {
			...
			this.$title.css({
				'line-height': this.options.titleHeight + 'px'
			});
		...
		close: function (callback) {
			...
			var _close = function (callback) {
				that.animate({
					width: that.options.initialWidth,
					height: that.options.initialHeight,
					opacity: 0
				}, function () {
		...
		restore: function (callback) {
			if (this.minimized) {
				...
				this.animate(this.maximized, function () {
					...
					that.$icons.max
						.unbind('click.max.' + that.options.prefix)
						.one('click.max.' + that.options.prefix, function () {
							that.max();
						});
					that.enableDrag();
					that.enableResize();
...
$(function () {
	//샵 탭
	$('.gift-home-tab .tab-tit a').on('click', function () {
		if (!$(this).closest('li').hasClass('on')) {
			var _tabIdx = $(this).closest('li').index();
			$(this).closest('li').addClass('on').siblings('li').removeClass('on');
			$('.gift-home-tab .tab-cont .cont').eq(_tabIdx).addClass('on').siblings('.cont').removeClass('on');
		}
	});
	//테마샵 아이콘 버튼 피드백
	btnFeedbackEffect($('.gift-home-tab .tab-tit li a'));
	btnFeedbackEffect($('.gift-home-tab .tab-cont li a'));
	btnFeedbackEffect($('.gift-home-list li a'));

	//감동선물 탭
	$('.gift-home-slide').each(function () {
		var $parent = this;
		var $tab = $('.slide-tab', $parent);
		$('li', $tab).each(function () {
			var $list = $(this);
			var idx = $list.index();
			$("a", $list).on({
				'click': function () {
					$(".product-list-v2", $parent).removeClass('on');
					$('li', $tab).removeClass('on');
					$(".product-list-v2", $parent).eq(idx).addClass('on');
					$(this).parent().addClass('on');
				}
			});
		});
	...
	//감동선물 탭 넓이설정
	function giftSlideTabSet() {
		if ($(window).width() < 768) {
			$('.gift-home-slide').each(function () {
				var giftTabSize = $('.slide-tab li', this).length;
				var gifttabsWidth = 0;
				for (var i = 0; i < giftTabSize; i++) {
					gifttabsWidth = gifttabsWidth + $('.gift-home-slide .slide-tab li').eq(i).width() + 10;
				}
				$('.slide-tab ul', this).css({
					width: gifttabsWidth
				});
			});
		}
	...
	if ($('.gift-home-slide').length > 0) {
		$slider = $('.gift-home-slide .product-list-v2');
		...
		$slider.on("afterChange", function (slick, slide) {
			...
			var length = slide.slideCount;

			var paddingBuffer = function (interval, effect) {
				$(".slick-list", _this).animate({
					'padding-left': interval + 'px'
				}, 800, effect)
			}
			...
			if ($(window).width() < 768) {
				if (current == length - 1) {
					if (length % 2 != 0) {
						paddingBuffer(270, 'easeOutBack');
					}
				} else if (current == length - 2) {
					if (length % 2 == 0) {
						paddingBuffer(128, 'easeOutBack');
					}
				} else if (current == 0) {
					paddingBuffer(20, 'easeOutBounce');
				}
			}
		});
 ...
 */
$(function () {
	var $alimiSlide = $('.alimi_slide_wrap');

	$alimiSlide.on('initialized.owl.carousel changed.owl.carousel', function (owl) {
		$('.total', $('.alimiSlide-ctrl')).text(owl.item.count);
		var nowIdx = owl.item.index - ($(this).find('.cloned').length) / 2 + 1;
		if (nowIdx > owl.item.count) {
			$('.now', $('.alimiSlide-ctrl')).text(1);
		} else {
			$('.now', $('.alimiSlide-ctrl')).text(nowIdx);
		}
	});

	$alimiSlide.owlCarousel({
		loop: true,
		nav: true,
		dots: false,
		margin: 0,
	...
	$('.alimiSlide-ctrl button').on('click', function () {
		if (!$(this).hasClass('stop')) {
			$(this).text('play').addClass('stop');
			$alimiSlide.trigger('stop.owl.autoplay');
		} else {
			$(this).text('stop').removeClass('stop');
			$alimiSlide.trigger('play.owl.autoplay', [5000]);
		}
	});
});
...
/** ------------------------------------------
 *  Header
 *  - GNB(mobile), MY(pc/mobile)
 *   ------------------------------------------
 */
var checkPixel = function () {
	if ($(window).width() < 768) {
		return true
	} else {
		return false
...
var clickfeedback = function (obj) {
	$("a, button", obj).on({
		'touchstart': function () {
			$(this).addClass('active');
		},
...
var mobile_navigation = {
	obj: {
		target: '.mobile-nav-menu .inner dl',
		cs: '.mobile-nav-menu .inner dl .amwayCs',
		business: '.mobile-nav-menu .inner dl .business',
		callback: function (that) {
			return true
		}
	},
	set_hasChild: function () {
		$(">ul li", $(this.obj.cs)).each(function () {
			if ($(this).has("ul").length) {
				$(this).addClass("hasChild");
			}
		});
	},
	set_businessInitBg: function (obj) {
		obj.css({
			'background-position': 'left top',
			'background-repeat': 'repeat',
		})
	},
	set_businessInit: function (obj) {
		var _this = this;
		obj.each(function () {
			if ($(this).is(".collapsed")) {
				$("button.show span.txt", $(this)).text('전체 열기');
				var $list = $("li", obj)
				_this.hide_Menu($list);
				_this.set_businessInitBg(obj);
			} else {
				$("button.show span.txt", $(this)).text('전체 닫기');
			}
		});
	},
	exe_businessShow: function (obj) {
		var _this = this;
		obj.toggleClass("collapsed");
		_this.set_businessInit(obj);
	},
	exe_business: function () {
		var _this = this;
		var $category = $(".category", $(_this.obj.business));
		_this.set_businessInit($category);
		$("button.show").click(function () {
			_this.exe_businessShow($category);
		});
	},
	set_submenu: function (obj) {
		var _this = this;
		var li_size = $("li", obj).length;
		var ul_height = (Math.ceil(li_size / 2)) * 40;
		var li_idx = obj.index() + 1;

		/* // 2020-08-20 add 5번째 이후 메뉴 컨트롤
		var ul_height_add = 0;			
		if (  obj.parents('dd').hasClass('business') && li_idx > 4 ) { ul_height_add = 20;}
		// 2020-08-20 //

		$(">li", obj.parent()).each(function(){
			var idx = $(this).index()+1;			
			if (Math.ceil(idx/4) == Math.ceil(li_idx/4)){
				//$(this).css({'margin-bottom':ul_height+'px'});				
				$(this).css({'margin-bottom':ul_height+'px'}); // 2020-08-20
			}
		});

		$(">ul", obj).css({//하위메뉴 좌표체크 및 활성화
			//'top':(obj.height()*Math.ceil(li_idx/4))+'px'
			'top':(obj.height()*Math.ceil(li_idx/4))+ul_height_add+'px' // 2020-08-20
		}); */

		// 2020-08-20 add 비즈니스 5번째 이후 메뉴 컨트롤 
		$(">li", obj.parent()).each(function () {
			var idx = $(this).index() + 1;
			if (Math.ceil(idx / 4) == Math.ceil(li_idx / 4)) {
				$(this).css({
					'margin-bottom': ul_height + 'px'
				});
			}
		});

		if (obj.closest('dd').hasClass('business')) {
			var line1 = $(">li", obj.parent()).eq(1).height(),
				line2 = $(">li", obj.parent()).eq(5).height(),
				line3 = $(">li", obj.parent()).eq(9).height();

			if (li_idx <= 4) {
				$(">ul", obj).css({
					'top': line1 + 'px'
				});
			} else if (li_idx >= 5 && li_idx < 9) {
				$(">ul", obj).css({
					'top': line1 + line2 + 'px'
				});
			} else if (li_idx >= 9) {
				$(">ul", obj).css({
					'top': line1 + line2 + line3 + 'px'
				});
			}
		} else {
			$(">ul", obj).css({ //하위메뉴 좌표체크 및 활성화
		...
		/*bG 정리*/
		var row = Math.ceil(li_idx / 4);
		if (obj.parents(".shopping").length) {
			if (row == 1) {
				obj.parent().removeClass("line");
			} else if (row == 2) {
				obj.parent().addClass("line");
			}
		} else if (obj.parents(".business").length) {
			var interval = 110;
			if (row == 1) {
				obj.parent().css({
					'background-position': 'left ' + ((interval * row) + ul_height) + 'px',
	...
	hide_Menu: function (obj) {
		obj.each(function () { //초기화
			$(this).removeClass('on').css({
				'margin': 0
			});
			$("li", $(this)).removeClass('on').css({
				'margin': 0
			});
		});
	},
	exe_Menu: function () {
		var _this = this;
		$("ul li", $(_this.obj.target)).each(function () {
			var $li = $(this);
			var $ul = $li.parent();
			...
			$(">a", $li).click(function (e) {
				if ($li.has("ul").length > 0) {
					e.preventDefault();
					var statusChk = $li.is(".on") ? true : false;
					var $list = $(">li", $ul);

					_this.hide_Menu($list);

					/*bG line 정리*/
					if ($ul.parents(".shopping").length) {
						$(".category").addClass("line");
					} else if ($ul.parents(".business").length) {
						_this.set_businessInitBg($ul);
					}

					if (($ul.is(".category") || $(this).parents(".amwayCs").length) && statusChk) {
						return false
					} else if ($ul.is(".category") && !statusChk) {
						_this.set_submenu($li);
						$li.addClass('on');
					} else {
						$li.addClass('on');
					}
	...
	exe_Navigation: function () {
		// 2020-04-23 스크립트 수정
		var $body = $('body');
		$(".header-wrapper button.ham").click(function () {
		...
		$(".mobile-nav-menu button.close").click(function () {
...
// document 클릭시 스크롤되는 현상 관련 삭제
var mini_dashboard = {
	obj: {
		target: '.mini-dashboard .myAccount',
		dashboard: '#myAccount',
		callback: function (that) {
			return true
		},
	...
	exe_dashboard: function () {
		var _this = this;
		$(document).on({
			'click': function (e) {
				var isMobile = $(window).width() <= 768;

				if (e.target.className === 'btn_layer-close') {
					$('header').removeClass('userinfo-open');
					$('html, body').removeClass('scroll-off');
					$(_this.obj.dashboard).removeClass('on');
					return false
				}

				if (e.target.className === 'btn-qr') return false;
				// S : 20221014 미니대시보드 내부 팝업 노출 시 미니대시보드 노출 유지
				if (e.target.className === 'btn-subscribe') return false; 

				if (e.target.className === "btn-modal_close btn-subscribe_close") {
					$('header').addClass('userinfo-open');
					//$('html, body').addClass('scroll-off'); 20230209 수정
					$(_this.obj.dashboard).addClass('on');
					return false 
				}
				// E : 20221014 미니대시보드 내부 팝업 노출 시 미니대시보드 노출 유지

				// S : 20221031 미니대시보드 내부 팝업 노출 시 미니대시보드 노출 유지
				if (e.target.className === 'abc-patron-admin') return false; 
				if (e.target.className === 'abc-patron-admin color-skyblue') return false; 

				if (e.target.className === "btn-modal_close v2 abc-patron-close") {
					$('header').addClass('userinfo-open');
				...
				if (e.target.className === "btn-modal_apply abc-patron-close") {
					$('header').addClass('userinfo-open');
					$('html, body').addClass('scroll-off');
					$(_this.obj.dashboard).addClass('on');
					return false 
				}
				// E : 20221031 미니대시보드 내부 팝업 노출 시 미니대시보드 노출 유지

				if (e.target.className == 'myAccount js-my-account-menu' || $(e.target).parents(_this.obj.target).length > 0) {
					if ($(_this.obj.dashboard).is('.on')) {
						$('header').removeClass('userinfo-open');
						$(_this.obj.dashboard).removeClass('on');
						if (isMobile) {
							$('html body').removeAttr('style');
							$('html, body').removeClass('scroll-off');
							window.scrollTo(0, _this.obj.setPosition);
						}
					} else {
						_this.obj.setPosition = $(window).scrollTop();
						$(_this.obj.dashboard).addClass('on');
						if (isMobile) {
							$('html, body').addClass('scroll-off');
							$('html body').css({
								top: _this.obj.setPosition * -1,
							});
						}
					}
				} else {
					$('#header').find('header').removeClass('userinfo-open');
					$(_this.obj.dashboard).removeClass('on');
					if (isMobile) {
						$('html, body').removeClass('scroll-off');
						if (_this.obj.setPosition === 0) return true
						$('html body').removeAttr('style');
						window.scrollTo(0, _this.obj.setPosition);
					}
				}
			},
			// 'hover mouseenter':function(e){//pc gnb over 시에 이미 처리됨
			// }
		});
	},
	init: function () {
...
document.addEventListener('DOMContentLoaded', function (event) {
	if (/iPhone/.test(navigator.userAgent) && !window.MSStream) {
		var metaViewportTag = document.head.querySelector('meta[name="viewport"]');
		document.getElementsByTagName('meta').item(property = 'viewport');
...
// 최초 화면 진입시 팝업 노출 여부
// ($(function(){
// 	var layerVisible = ($('.layer_notify, .member-type').is(':visible'))// || ($('.member-type').is(':visible'));
// 	if(layerVisible) {
// 		console.log('레이어 보임')
// 		$('html, body').addClass('scroll-off');
// 		// $('.js-not-open-today').on('click', )
// 	} else {
// 		console.log('안보임');
// 		$('html, body').removeClass('scroll-off');
// 	}
...
$(function () {
	$('.renewal_my-shopping .myorder-list.coustom-type').siblings().find('p').addClass('myorder-list_count'); /* 전체주문내역 내역 없을 경우 문구 위치 수정(ASB-9461) */
	//$('.kakao_share-product').parents().find('#loadLayer2').addClass('kakaoCart'); /* 20221019 A Clicks 관련 내용 추가 */

	/* S : 20221101 마이페이지 전체주문내역 필터, 검색 버튼 개선 */
	$('.myorder-history .order-conts .myorder button').on('click', function(){
		$(this).toggleClass('on').siblings().removeClass('on');
	});
	/* E : 20221101 마이페이지 전체주문내역 필터, 검색 버튼 개선 */

	/* S : 20221115 결제화면 BG 영역 색상 수정 */
	$(document).on('click', '#target_payment a.payTaba', function(){
		/* S : 20221228 수정 */
		if ($(this).is('.payTab1, .payTab3')) {
			$(this).parents('.accodion_box_cont').css('background-color', '#f4f4f4');
			$(this).parents('.accodion_box_cont').siblings().css('background-color', '#fff');
		} else if ($(this).is('.payTab2, .payTab4, .payTab5')) {
			$(this).parents('.accodion_box_cont').css('background-color', '#f4f4f4');
			$(this).parents('.accodion_box_cont').siblings().css('background-color', '#fff');
		}
		/* E : 20221228 수정 */
	});
	/* E : 20221115 결제화면 BG 영역 색상 수정 */

	/*
	SOP 상품페이지에만 적용 / 제품 상세 롤링 영역 테스트 작업(/_ui/responsive/html/shop/renewal/G10300.html)
	var pdp_swiper = new Swiper('#SOPTabPanelContainer .product-detail .card_swiper_wrap .card_section', {
		loop: true,
		pagination: {
	...
	*/
	...
	/* S : 20230405 A Pay 현장결제용 QR코드 팝업창 닫으면 미니대시보드 열리는 오류 수정(Sprint4, 상시모니터링 No39) */
	/* S : 20230102 미니대시보드 나의 QR코드 팝업 후, 스크롤 오류 수정 (Sprint1, 상시모니터링 No2) */
	// $(document).on('click', '.layer-qr-wrap.qr-renew .btn_layer-close', function(){
	// 	$('header').addClass('userinfo-open');
	// 	$('html, body').addClass('scroll-off');
	...
	$(document).on('click', '#myAccount .btn-qr', function(){
		$('#myAccount .layer-qr-wrap.qr-renew .btn_layer-close').on('click', function(){
			$('header').addClass('userinfo-open');
			$('html, body').addClass('scroll-off');
			$('#myAccount').addClass('on');
			return false;
	...
	/* E : 20230102 미니대시보드 나의 QR코드 팝업 후, 스크롤 오류 수정 (Sprint1, 상시모니터링 No2) */
	/* E : 20230405 A Pay 현장결제용 QR코드 팝업창 닫으면 미니대시보드 열리는 오류 수정(Sprint4, 상시모니터링 No39) */

	/* S : 20230209 무이자 할부 정보 제공 팝업 스크립트 추가 */
	$(".card_benefit-area .card_benefit1").on("click", function(){
		if ($(".card_benefits .tabs-toggles li").hasClass("active")){
			$(".card_benefits .tabs-toggles li").eq(0).addClass("active");
			$(".card_benefits .tabs-toggles li").eq(1).removeClass("active");
		} 

		if ($(".card_benefits .tab-pane.content-block").hasClass("active")){
			$(".card_benefits .tab-pane.content-block").eq(0).addClass("active");
			$(".card_benefits .tab-pane.content-block").eq(1).removeClass("active");
		}

		$(".layer-area_card_benefits").scrollTop(0);

	})

	$(".card_benefit-area .card_benefit2").on("click", function(){
		if ($(".card_benefits .tabs-toggles li").hasClass("active")){
			$(".card_benefits .tabs-toggles li").eq(0).removeClass("active");
			$(".card_benefits .tabs-toggles li").eq(1).addClass("active");
		} 

		if ($(".card_benefits .tab-pane.content-block").hasClass("active")){
			$(".card_benefits .tab-pane.content-block").eq(0).removeClass("active");
			$(".card_benefits .tab-pane.content-block").eq(1).addClass("active");
		}

		$(".layer-area_card_benefits").scrollTop(0);
	})
	/* E : 20230209 무이자 할부 정보 제공 팝업 스크립트 추가 */

	/* S : 20230215 무이자 할부 정보 제공 팝업 스크립트 추가 수정 */
	// $(".layer-area_card_benefits").scroll(function(){
	// 	$('html, body').addClass('scroll-off');
	// })
	/* E : 20230215 무이자 할부 정보 제공 팝업 스크립트 추가 수정 */

	/* S : 20230807 로그미 서비스 관련 js 추가 */
	$('.input_pinNum').each(function(pidx){
		const inpPinNum = $(this).addClass('pinNum' + pidx);

		/* S: 20231016 수정 */
		inpPinNum.find('input[name=inp_pwd]').on('input', function(){
			var inputValLength = inpPinNum.find('> input').val().length;
			var inp_mark_ico = inpPinNum.find('.inp_mark_box .inp_mark_ico');

			if (inputValLength == 0) {
				inpPinNum.attr('data-count', 0);
				inp_mark_ico.eq(0).addClass('logmeAct').removeClass('logmeOn');
				inp_mark_ico.eq(1).removeClass('logmeAct');
			} else if (inputValLength == 1) {
				inpPinNum.attr('data-count', 1);
				inp_mark_ico.eq(0).removeClass('logmeAct').addClass('logmeOn');
				inp_mark_ico.eq(1).addClass('logmeAct').removeClass('logmeOn');
				inp_mark_ico.eq(2).removeClass('logmeAct');
			} else if (inputValLength == 2) {
				inpPinNum.attr('data-count', 2);
				inp_mark_ico.eq(1).removeClass('logmeAct').addClass('logmeOn');
				inp_mark_ico.eq(2).addClass('logmeAct').removeClass('logmeOn');
				inp_mark_ico.eq(3).removeClass('logmeAct');
			} else if (inputValLength == 3) {
				inpPinNum.attr('data-count', 3);
				inp_mark_ico.eq(2).removeClass('logmeAct').addClass('logmeOn');
				inp_mark_ico.eq(3).addClass('logmeAct').removeClass('logmeOn');
			} else if (inputValLength == 4) {
				inpPinNum.attr('data-count', 4);
				inp_mark_ico.eq(3).removeClass('logmeAct').addClass('logmeOn');
			}
		});
		/* E: 20231016 수정 */
	});

	/* S : 20231016 로그미 PIN 인증 팝업 오픈 시 input focus */
	let logmePINPopBtn = document.querySelectorAll(".logmePinPopBtn");

	logmePINPopBtn.forEach(function(logmeCurrBtn){
		logmeCurrBtn.addEventListener("click", logmeInputFocus);
	});

	function logmeInputFocus(){			
		setTimeout(function(){
			let logmeInputArr = $("input[name='inp_pwd']");
	...
	/* E : 20231016 로그미 PIN 인증 팝업 오픈 시 input focus */
	/* E : 20230807 로그미 서비스 관련 js 추가 */

	/* S : 20231006 모바일 Copyright영역 크기 조정 */
	function mobile_floatingBtn(){
		if($(window).width() <= 768) {
			if( $('.box_mobile-floating').length ){ // 하단 플로팅 버튼이 있을 경우
				if( $('.renewal_floating-type').length ) {
					$('#footer').css('padding-bottom', '55px');
				} else if( $('.renewal_floating-type2').length ) {
					$('#footer').css('padding-bottom', '100px');

					/* S : 20231012 모바일 장바구니 화면 하단 가격표시 개선 */
					if( $('.cart_product-area').find('.box_total-floating_new') ) {
						$('#footer').css('padding-bottom', $('.box_mobile-floating').height());
					}
					/* E : 20231012 모바일 장바구니 화면 하단 가격표시 개선 */
				} else {
					$('#footer').css('padding-bottom', $('.box_mobile-floating button').height());
				}
			
				if( $('.box_mobile-floating').hasClass('type_sop') || $('.box_mobile-floating').hasClass('payment') ){
					$('#footer').css('padding-bottom', '0');
				}
			} else {
				if( $('.article-c-basket-sop').length ){ // 장스오 내역 없을 경우
					$('#footer').css('padding-bottom', '0');
				}
			}
		} else {
			$('#footer').css('padding-bottom', '0'); // 20231101 추가
		}
	}
	...
	//input 입력 된 검색어 삭제 버튼 노출
	// const searchInput = $(".cancelBtn-box input");
	// const searchDelBtn = $(".cancelBtn-box .cancelBtn");
	// const searchSuggest = $(".item-suggestion-popover"); /* 20231020 검색어 추천 영역 노출 관련 스크립트 현행화 */

	// searchInput.on("keyup",function(){
	// 	/* S : 20231012 수정 */
	// 	if ($(this).val() !== "") {
	// 		delBtnShow();
	// 		searchSuggest.show(); /* 20231020 검색어 추천 영역 노출 관련 스크립트 현행화 */
	// 	} else if ($(this).val() == "") {
	// 		delBtnHide();
	// 	};
	// 	/* E : 20231012 수정 */
	...
	// 	delBtnHide();
	// 	searchInput.val("");
	// 	searchInput.focus(); /* 20231017 추가 */
	// 	searchSuggest.hide(); /* 20231020 검색어 추천 영역 노출 관련 스크립트 현행화 */
	// });

	// function delBtnShow() {
	// 	searchDelBtn.children().css("display", "block");
	// };

	// function delBtnHide() {
	// 	searchDelBtn.children().css("display", "none");
	// };

	/* S : 20231020 검색 영역 X버튼 동적 추가 되어도 기능 작동 되도록 js 수정 */
	//input 입력 된 검색어 삭제 버튼 노출
	$(document).on("keyup", ".cancelBtn-box .inp_item_quick_sch, .cancelBtn-box .js-site-search-input",function(){
		if ($(this).val() !== "") {
			delBtnShow($(this));
			$(".item-suggestion-popover").show();
		} else if ($(this).val() == "") {
			delBtnHide($(this));
		};
	});

	$(document).on("click", ".cancelBtn-box .cancelBtn",function(){
		delBtnHide($(this));
		$(this).parent().find(".inp_item_quick_sch, .js-site-search-input").val("");
		$(this).parent().find(".inp_item_quick_sch, .js-site-search-input").trigger("keyup").focus();
		$(".item-suggestion-popover").hide();
		$(".item-suggestion-popover").find("ul").html("")
	});

	function delBtnShow(searchInput) {
		$(searchInput).parent().find(".icon-cancel").css("display", "block");
	};

	function delBtnHide(searchInput) {
		$(searchInput).parent().find(".icon-cancel").css("display", "none");
	};
	/* E : 20231020 검색 영역 X버튼 동적 추가 되어도 기능 작동 되도록 js 수정 */
	/* E : 20230919 최근 검색어 개선 */

	// 20240108 일부 페이지에서는 상품을 각각 다른 컴포넌트로 불러오고 있어서 해당 스크립트가 적용 안되고 있어서 삭제, css로만 적용되도록 수정
	/* S : 20231023 Flag 갯수 체크(3개 이상일 경우 클래스 추가) */
	// $('.box_product.product_list-type .product_item').each(function(){
	// 	if( $(this).find('.flag_wrap span').length >= 3 ) {
	// 		$(this).find('.flag_wrap').addClass('flag_wrap-over');
	// 	}
	// });
	/* E : 20231023 Flag 갯수 체크(3개 이상일 경우 클래스 추가) */

	/* S : 20231030 최근검색어 토글 버튼 클릭 시 텍스트 변경 */
	$(".new-search-cont.renewal .switch_toggle .btn_toggle").on("click", function(){
		if ($(this).siblings("input:checkbox[id='switch_toggle']").is(":checked") == true) {
			$(this).children().text("최근검색어 OFF"); /* 20231127 수정 */
		} else {
			$(this).children().text("최근검색어 ON"); /* 20231127 수정 */
		}
	});
	/* E : 20231030 최근검색어 토글 버튼 클릭 시 텍스트 변경 */

	/* S : 20240315 PDP 시인성 개선 - 모바일 floating 영역 */
	var moFloatingBtn = $(".box_mobile-floating.floating_renew > .btn_floating-group .btn_box button");
	var moFloatingDetailBox = $(".orderQtySelect_pop");
	var moFloatingDetailBtnBox = $(".orderQtySelect_pop .floating-detail-btn .btn_box");
	...
	$(moFloatingBtn).on("click", function(event){
		...
		if (moFloatingBtnID !== undefined) {
			...
			if (moFloatingBtnID == "btn_aCliks") {
				$(moFloatingDetailBtnBox).find("#btn_aCliks").parent().css("display", "block");
			} else if (moFloatingBtnID == "btn_gift") {
				$(moFloatingDetailBtnBox).find("#btn_gift").parent().css("display", "block");
				$(moFloatingDetailBtnBox).find(".btn-basket").parent().css("display", "block");
			} else if (moFloatingBtnID == "btn_purchase") {
	...
	$(".close_dim").on("click", function(e){
		e.preventDefault();
		$(this).parents().removeClass("active").attr("style","");
		$("#uiLayerMask").hide().removeClass("on");
		$("html").removeClass("scrollLock");
	});
	...
	$(window).on("resize", function(){
		if($(window).width() > 768) {
			if($(moFloatingDetailBox).hasClass("active")) {
				$(moFloatingDetailBox).removeClass("active").attr("style","");
				$("#uiLayerMask").hide().removeClass("on");
				$("html").removeClass("scrollLock");
			}
		};
