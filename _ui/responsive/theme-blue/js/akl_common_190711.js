...
function HEADER_stopPropagation(){
	...
	$(document).on('click',function(){
		...
		//쇼핑메뉴 레이어 숨기기 190409 삭제
		// if( $('.overlay-menu-toggle-desktop').not('.collapsed') && $("[id^=overlay-menu-wrapper]").is('.in') ){
		// 	$("[id^=overlay-menu-wrapper]").removeClass("in");
		// 	$('.overlay-menu-toggle-desktop').addClass('collapsed');
		// 	// $('#uiLayerMask').removeClass('on gnb_open').hide();// 190328 마스크 숨김
		// }
	});
}
...
function HEADER_MenuShop(){
	...
	// 190328 메뉴 클릭시 마스크 노출 기능 추가
	// $('.overlay-menu-toggle-mobile,.overlay-menu-toggle-desktop').on('click',function(){
	// 	$('#uiLayerMask').toggleClass('on gnb_open').toggle();
	// });
...
function navToggling() {
	...
	if(window.innerWidth < 768) {
		...
		newRoot.find('.subcategory-toggle').click(function(e) { // 3depth event
		...
		twoDepth.click(function (e) { // 2depth event
		...
		oneDepth.click(function (e) { // 1depth event
...
function GNB_SubDepth_Control(){
	...
	// 	var notMenu = $(e.target).closest('.top-menu-links.desktop-structure-modified').length == 0;
	// 	if (notMenu) {
	// 		oneDepthLink.addClass('collapsed');
	// 		oneDepthCon.removeClass('in');
	// 	}
